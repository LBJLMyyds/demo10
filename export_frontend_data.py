...
def export():
    ...
    # Schema note: as of this update, conflict_candidates is produced directly by
    # src/pipeline/clean.py::build_conflict_candidates(), which now does its own
    # recency-weighted resolution (exponential half-life decay over review_id-linked
    # evidence only -- the place-level aggregate row is deliberately excluded so it
    # can't be double-counted as an extra "vote"). status is either "provisional"
    # (engine is confident enough to propose a value) or "human_review" (evidence
    # too sparse/mixed to trust automatically). There is no third "resolved_*"
    # bucket like an earlier draft of this script assumed -- only these two.
    cur.execute("SELECT * FROM conflict_candidates")
    conflict_rows = [dict(r) for r in cur.fetchall()]
    # Only conflicts the engine couldn't confidently resolve should make a venue
    # show a "needs review" banner on the map. Provisional conflicts still appear
    # in full on the Data Quality page (for audit purposes) but shouldn't alarm
    # someone just trying to find an accessible venue.
    flagged_place_ids = {r["place_id"] for r in conflict_rows if r["status"] == "human_review"}
    ...
    # ---- individual reviews, grouped by place (the "specific reviews" behind
    # every aggregate number and every conflict-engine decision) ----
    cur.execute("""
        SELECT review_id, place_id, rating, steps_on_entry, stable_ramp, acs_bathroom,
               acs_seating, acs_parking, reviewed_at
        FROM reviews
        ORDER BY reviewed_at DESC
    """)
    reviews_by_place: dict[str, list[dict]] = {}
    for r in cur.fetchall():
        reviews_by_place.setdefault(r["place_id"], []).append({
            "id": r["review_id"],
            "rating": r["rating"],
            "stepsOnEntry": r["steps_on_entry"],
            "features": {
                "ramp": r["stable_ramp"],
                "bathroom": r["acs_bathroom"],
                "seating": r["acs_seating"],
                "parking": r["acs_parking"],
            },
    ...
    for p in place_rows:
        ...
        places_out.append({
            ...
            "reviews": reviews_by_place.get(p["place_id"], []),
    ...
    # notes in the DB is a constant methodology disclaimer (same text on every row),
    # not a per-conflict rationale, so build a specific one here from the actual
    # numbers -- this is what the frontend displays per row.
    def build_rationale(c: dict) -> str:
        quality_label = {"high": "high confidence", "medium": "medium confidence", "low": "low confidence"}.get(
            c["evidence_quality"], c["evidence_quality"])
        score_pct = f"{c['resolution_score']:.0%}" if c["resolution_score"] is not None else "n/a"
        if c["status"] == "provisional":
            when = ""
            if c["latest_reviewed_at"]:
                try:
                    when = f" — latest evidence {datetime.fromisoformat(c['latest_reviewed_at']).strftime('%d %b %Y')}"
                except ValueError:
                    when = ""
            return (f"Engine proposes '{c['proposed_value']}' ({quality_label}, resolution score {score_pct})"
                    f"{when}.")
        return (f"Evidence too sparse or mixed to resolve automatically ({quality_label}, "
                f"resolution score {score_pct}) — left for human review.")
    ...
    for c in conflict_rows:
        ...
        conflicts_out.append({
            ...
            "datedEvidenceCount": c["dated_evidence_count"],
            "yesWeight": round(c["yes_weight"], 3) if c["yes_weight"] is not None else None,
            "noWeight": round(c["no_weight"], 3) if c["no_weight"] is not None else None,
            "unsureWeight": round(c["unsure_weight"], 3) if c["unsure_weight"] is not None else None,
            "proposedValue": c["proposed_value"],
            "resolutionScore": round(c["resolution_score"], 4) if c["resolution_score"] is not None else None,
            "evidenceQuality": c["evidence_quality"],
            "latestReviewedAt": c["latest_reviewed_at"],
            "latestReviewIds": [rid for rid in (c["latest_review_ids"] or "").split(";") if rid],
            "status": c["status"],
            "rationale": build_rationale(c),
        })
    ...
    # ---- conflict resolution engine summary (coverage / human-review-rate) ----
    # These two don't need ground truth. resolution *accuracy* -- whether a
    # provisional value actually matches reality -- still isn't measured; that
    # needs a labelled ground-truth sample that doesn't exist yet (see
    # docs/metric_validation_plan.md).
    status_counts: dict[str, int] = {}
    quality_counts: dict[str, int] = {}
    for c in conflict_rows:
        status_counts[c["status"]] = status_counts.get(c["status"], 0) + 1
        quality_counts[c["evidence_quality"]] = quality_counts.get(c["evidence_quality"], 0) + 1
    total_conflicts = len(conflict_rows)
    needs_review = status_counts.get("human_review", 0)
    resolution_out = {
        "method": "review_recency_weighted",
        "totalConflicts": total_conflicts,
        "byStatus": status_counts,
        "byEvidenceQuality": quality_counts,
